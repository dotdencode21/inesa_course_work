<template>
<svg version="1.1" id="Icons" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 32 32" width="82" height="82" xml:space="preserve">
  <path d="M25,28h-1.6c-0.3-4.6-2.4-9-5.8-12c3.4-3,5.5-7.4,5.8-12H25c0.6,0,1-0.4,1-1s-0.4-1-1-1H7C6.4,2,6,2.4,6,3s0.4,1,1,1h1.6
	  c0.3,4.6,2.4,9,5.8,12c-3.4,3-5.5,7.4-5.8,12H7c-0.6,0-1,0.4-1,1s0.4,1,1,1h18c0.6,0,1-0.4,1-1S25.6,28,25,28z M12.3,10h7.4
	  c-0.9,1.8-2.2,3.4-3.7,4.7C14.5,13.4,13.2,11.8,12.3,10z M11.5,28l3.7-4.6c0.4-0.5,1.2-0.5,1.6,0l3.7,4.6H11.5z" :fill="fill" />
</svg>
</template>
<script>
export default {
  props: {
    fill: {
      type: String,
      default: "#000000"
    }
  }
};
</script>